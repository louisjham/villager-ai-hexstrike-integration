...
# Ensure Villager comes before HexStrike in ~/.cursor/mcp.json
...
**Symptoms**: Tasks created but don't execute or fail
...
# Check Kali driver connectivity
...
docker run -d --name test-kali kalilinux/kali-rolling /bin/bash
...
docker stop test-kali && docker rm test-kali
```
...
*For additional help, see the [Setup Guide](SETUP_GUIDE.md) or [AI Assistant Guide](AI_ASSISTANT_GUIDE.md).*
